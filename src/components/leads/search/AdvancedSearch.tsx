...
export const AdvancedSearch = ({
  ...
  const departmentGroups = [{
    ...
    options: [{
      ...
      value: "scrum_master_agile_coach"
    }, {
      label: "Software Development",
      value: "software_development"
    }, {
      label: "Support / Technical Services",
      value: "support_technical_services"
    }, {
      label: "Technician",
      value: "technician"
    }, {
      label: "Technology Operations",
      value: "technology_operations"
    }, {
    ...
    options: [{
      ...
      value: "culture_diversity_inclusion"
    }, {
      label: "Employee & Labor Relations",
      value: "employee_labor_relations"
    }, {
      label: "Health & Safety",
      value: "health_safety"
    }, {
    ...
    options: [{
      ...
      value: "labor_employment"
    }, {
      label: "Lawyer / Attorney",
      value: "lawyer_attorney"
    }, {
      label: "Legal",
      value: "legal"
    }, {
    ...
    options: [{
      ...
      value: "customer_marketing"
    }, {
      label: "Demand Generation",
      value: "demand_generation"
    }, {
      label: "Digital Marketing",
      value: "digital_marketing"
    }, {
      label: "Ecommerce Marketing",
      value: "ecommerce_marketing"
    }, {
      label: "Event Marketing",
      value: "event_marketing"
    }, {
  ...
  const handleToggleEmailStatus = (value: string) => {
    setEmailStatus(prev => prev.includes(value) ? prev.filter(item => item !== value) : [...prev, value]);
  };
  const handleToggleSeniority = (value: string) => {
    setSeniorities(prev => prev.includes(value) ? prev.filter(item => item !== value) : [...prev, value]);
  };
  const handleToggleEmployeeRange = (value: string) => {
    setEmployeeRanges(prev => prev.includes(value) ? prev.filter(item => item !== value) : [...prev, value]);
  };
  ...
  const handleSearch = () => {
    ...
    departmentGroups.forEach(group => {
      const selectedInGroup = group.options.filter(opt => departments.includes(opt.value)).map(opt => opt.value);
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Input id="location" value={location} onChange={e => setLocation(e.target.value)} placeholder="Country or region e.g., United States" />
        </div>
        ...
        <div>
          ...
          <Select value={resultCount} onValueChange={setResultCount}>
            ...
            <SelectContent>
              {resultCountOptions.map(option => <SelectItem key={option.value} value={option.value}>
                  {option.label}
                </SelectItem>)}
            </SelectContent>
          </Select>
            ...
            <div className="space-y-6 pt-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-4">
                  {emailStatusOptions.map(option => <div key={option.value} className="flex items-center space-x-2">
                      <Checkbox id={`email-status-${option.value}`} checked={emailStatus.includes(option.value)} onCheckedChange={() => handleToggleEmailStatus(option.value)} />
              ...
              <div className="space-y-2">
                ...
                <div className="max-h-80 overflow-y-auto border rounded-md p-4">
                  {departmentGroups.map(group => <DepartmentFilterGroup key={group.label} label={group.label} defaultValue={group.defaultValue} options={group.options} selectedValues={departments} onChange={handleDepartmentsChange} />)}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {employeeRangeOptions.map(option => <div key={option.value} className="flex items-center space-x-2">
                      <Checkbox id={`employee-range-${option.value}`} checked={employeeRanges.includes(option.value)} onCheckedChange={() => handleToggleEmployeeRange(option.value)} />
